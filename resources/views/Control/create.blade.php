...
<br><br>
    <form  method="POST" action="{{route('Control.store')}}" id="form1" style="display:none" class="w-1/2 mx-auto " enctype="multipart/form-data">
        ...
        <input type="text" id=""  name="sectionName" class=" my-2 shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" placeholder="اسم القسم ... " required />
    <button type="submit" class="text-white bg-bro hover:bg-bro/80 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">انشاء</button>
  </form>
<form  method="POST" action="{{route('posts.store')}}" id="form2" style="display:none" class="w-1/2 mx-auto " enctype="multipart/form-data">
    ...
    <option name="ids"  value="{{$sec['id']}} "  > {{$sec['section_Name']}}</option>
        ...
        <input type="text" id="title_art"  name="title_art" class="btninput" placeholder="العنوان"  />
        <br>
...
<br>
<input type="file"  style="display: none" id="fileVid" accept="video/*"  name="fileVid" class="btninput" placeholder="العنوان"  />
